...
因此自定义提供方下的视觉模型只需一处声明：在 Models 页面展开该模型的**模型详情**折叠区并勾选**接受图片**，即会为该模型写入 `input: [text, image]`。取消勾选写入的是 `input: [text]`，而不是删除该字段，因此默认接受图片的路由不会为你声明为纯文本的模型重新放行图片。该字段也可以直接写在 `$DSH_HOME/settings.yaml` 中：
...
- **图片在发送前被拒绝**：该模型未声明图片模态。请在自定义提供方的模型上勾选**接受图片**，或给它加上 `input: [text, image]`；在 DeepSeek 自身的路由上，请从配置的目录中选择支持图片的条目（默认为 `deepseek-flash`），并确认网关提供该模型且支持图片输入。
