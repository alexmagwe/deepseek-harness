...
**pi-ai 模型列表编辑器会编辑 `input`；其余 settings 文档字段仍不编辑。** `compat`、`reasoningEfforts`、`thinkingBudgets`、`headers` 都没有编辑器，而 [Models 页面的图片声明](../feature/2026-09-07-model-editor-image-modalities.zh.md)拥有写入该字段的唯一复选框。那张卡片仍是一张只覆盖 id、名称和两个容量的手写表单，这不带来持久代价：它的行 patch 会先展开已存储的行再应用改动，而采纳候选时已有行优先于重新发现的候选，因此它不编辑的字段在两条路径上都能存活。
...
自定义提供方下的视觉模型只需在 settings 文档里写一行 `input: [text, image]`，或在 Models 页面勾选一个复选框——或者当该路由列出的模型全都接受图片时，在路由上写一行。这就是修复的全部：三个准入点随即接纳它上面的图片，`read_image` 也可用。什么都不写的部署保持原有行为完全不变，因此没有任何既有路由改变它报出的内容。
